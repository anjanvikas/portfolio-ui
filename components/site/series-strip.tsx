...
export function SeriesStrip({
  ...
  return (
    <div className="border-y-2 border-ink bg-accent-2">
      <div className="mx-auto max-w-4xl px-5 py-4 md:px-8 md:py-6">
        ...
        >
          <span className="font-mono text-xs font-bold uppercase tracking-wider text-ink md:text-sm">
            {heading}
          </span>
          <ChevronDown
            strokeWidth={2.5}
            className={`h-4 w-4 shrink-0 text-ink transition-transform md:hidden ${
              open ? "rotate-180" : ""
            }`}
            aria-hidden
          />
          ...
          {series.posts.map((post) => {
            ...
            if (isCurrent) {
              return (
                <li
                  key={post.slug}
                  className="flex items-center justify-between gap-3 border-l-[3px] border-ink bg-accent px-3 py-1.5"
                >
                  <span className="font-mono text-xs font-bold text-ink md:text-sm">
                    {label}{" "}
                    <span className="text-muted-brut">← (you)</span>
                  </span>
            ...
            return (
              <li key={post.slug}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="flex items-center justify-between gap-3 px-3 py-1.5 transition-colors hover:bg-ink/5"
                >
                  <span className="flex items-center gap-1.5 font-mono text-xs text-ink md:text-sm">
                    <Check strokeWidth={3} className="h-3.5 w-3.5 text-ink" aria-hidden />
                    {label}
                  </span>
                  <span className="shrink-0 font-mono text-[11px] text-ink/70">
                    {post.published_at}
